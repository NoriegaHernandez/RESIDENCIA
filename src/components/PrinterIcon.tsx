...
import { Printer as PrinterIconSvg } from 'lucide-react';
...
interface PrinterIconProps {
  ...
  onDragStart: () => void;
  onDragEnd: () => void;
  onDrag: (printerId: string, deltaX: number, deltaY: number) => void;
  isDragging: boolean;
}
...
export function PrinterIcon({
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    onDrag(printer.id, deltaX, deltaY);
    setDragStart({ x: event.clientX, y: event.clientY });
  };
  ...
  return (
    <div
      ...
      style={{
        left: `${printer.x_position}%`,
        top: `${printer.y_position}%`,
      }}
